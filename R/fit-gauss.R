...
#' [median()] and [mad()]. If `mad = TRUE`, these starting values are returned.
#'
#' @param x A numeric vector of numeric values to fit to Gaussian assumptions.
#' @param mad Logical. Should [median()] and [mad()] `* 1.4826` be used as
#' robust (i.e. distribution-free) estimates of population mean and
#' standard deviation?
#' @return A named vector consisting of non-linear least-squares
#' estimates of `mu` and `sigma` for `x`.
#' @author Stu Field
#' @seealso [nls()], [pnorm()]
#' @examples
#' x <- rnorm(100, 25, 3)
...
fit_gauss <- function(x, mad = FALSE) {
  ...
  if ( mad ) {
    pars          # return dist-free estimates (non-parametric)
  } else {
    # robust fit
    fit <- nls(
