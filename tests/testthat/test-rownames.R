...
test_that("the rowname helpers move rownames safely", {
  new <- rn2col(mtcars)    # default `name`
  expect_s3_class(new, "data.frame")
  expect_equal(new$.rn, rownames(mtcars))
  expect_true(".rn" %in% names(new))
  expect_equal(.row_names_info(new, type = 0L), c(NA, -32)) # now implicit rn
  expect_true(has_implicit_rn(new))                          # now implicit rn

  # `name` argument
  new <- rn2col(mtcars, "foo")
  expect_s3_class(new, "data.frame")
  expect_equal(new$foo, rownames(mtcars))
  expect_true("foo" %in% names(new))
  expect_equal(.row_names_info(new, type = 0L), c(NA, -32)) # now implicit rn
  expect_true(has_implicit_rn(new))                          # now implicit rn

  # moving columns
  expect_warning(x <- col2rn(mtcars, "id"))   # over-write warning
  expect_s3_class(x, "data.frame")
  expect_false("id" %in% rownames(x))
  ...
  # the make unique feature ensures unique rownames
  foo <- data.frame(a = 1:2, b = rep("A", 2L))
  expect_equal(rownames(col2rn(foo, "b")), make.unique(foo$b, "-"))
})

test_that("the rowname helpers have object fidelity", {
  df <- rn2col(mtcars) |> col2rn()  # convert & convert back
